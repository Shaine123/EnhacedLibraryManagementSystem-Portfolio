...
import StatMostLoggedTable from '../../../layout/table/StatMostLoggedTable'
...
const MostLogged = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3002/getStudent')
    .then(res => {
        setData(res.data)
    })

    axios.get('http://localhost:3002/getMostLoggedUser')
    .then(res => {
        setBook(res.data)
  ...
  const bookInfo = books.map((item) => {
    let temp = data.filter((val) => {
      return val.studentname == item.logName
   })
     if(temp.length > 0){
...
const filtered = bookInfo.filter(value => value !== undefined)
console.log(filtered)
  
  return (
    <>
       <nav className='dash-container'> 
       <h1>List of Most Logged Student</h1>
      ...
      <div className="title-header">
        {/* <h1>List of Most Logged Student</h1> */}
        <p>View the list of the most logged student</p>
      </div>
      ...
      <h2>Most Logged</h2>
       <StatMostLoggedTable data = {filtered} month = {month}/>
      </div>
    </>
  )
}
